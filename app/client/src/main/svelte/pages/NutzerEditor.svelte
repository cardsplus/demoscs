<script>
  ...
  $: disabled =
    !newNutzer.name || !newNutzer.mail || !newNutzer.allSprache.length;
...
<div class="py-4">
  {#if showUpdate}
    <Button on:click={() => onUpdateNutzer()} {disabled}>Ok</Button>
  {:else}
    <Button on:click={() => onCreateNutzer()} {disabled}>Ok</Button>
  {/if}
  {#if showRemove}
    <Button on:click={() => onRemoveNutzer()}>Löschen</Button>
  {/if}
