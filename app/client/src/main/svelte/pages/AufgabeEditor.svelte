...
<div class="py-4">
  {#if showUpdate}
    <Button on:click={() => onUpdateAufgabe()} {disabled}>Ok</Button>
  {:else}
    <Button on:click={() => onCreateAufgabe()} {disabled}>Ok</Button>
  {/if}
  {#if showRemove}
    <Button on:click={() => onRemoveAufgabe()}>Löschen</Button>
  {/if}
