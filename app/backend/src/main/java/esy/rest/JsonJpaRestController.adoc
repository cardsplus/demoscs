...
Mit einer Projection kann eine Ansicht (engl. view) auf eine Ressource gelegt werden.
...
TIP: Siehe
https://docs.spring.io/spring-data/rest/docs/current/reference/html/#projections-excerpts[Spring Data REST Reference Guide]
im Abschnitt _Excerpts_ für mehr Informationen.

tbd CORS

TIP: Siehe
https://docs.spring.io/spring-data/rest/docs/current/reference/html/#customizing-sdr.configuring-cors[Spring Data REST Reference Guide]
im Abschnitt _Configuring CORS_ für mehr Informationen.

tbd Normierte Statuscodes

TIP: Siehe
https://docs.spring.io/spring-data/rest/docs/current/reference/html/#repository-resources.default-status-codes[Spring Data REST Reference Guide]
im Abschnitt _Default Status Codes_ für mehr Informationen.
Abhängig von der Art der Operation gibt es noch zusätzliche Informationen zu _Custom Status Codes_ in den folgenden Abschnitten.
...
.Web-Socket
****
Nachrichten mit Daten, die häufig aktualisiert werden, können via Web-Socket an den Client in diesem SCS übertragen werden.
Damit können Nutzer durch den Client aktiv auf Veränderungen in den Daten hingewiesen werden.
Konflikte durch gleichzeitige Bearbeitung können dadurch entschärft werden.
****

.Kafka-Topic
****
Nachrichten mit Daten, für die dieses SCS die Hoheit besitzt, können in ein Kafka-Topic geschrieben werden.
Andere SCS werden so kontinuierlich mit aktuellen Daten aus diesem SCS versorgt.
****
